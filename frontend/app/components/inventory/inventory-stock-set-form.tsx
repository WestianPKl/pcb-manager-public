...
interface InventoryStockSetFormProps {
	inventoryId: string
...
const getErrorMessage = (error: unknown): string | string[] => {
	...
	return 'Failed to set stock'
}

export default function InventoryStockSetForm({ inventoryId, onSubmit }: InventoryStockSetFormProps) {
	const [isSubmitting, setIsSubmitting] = useState(false)
	const [quantity, setQuantity] = useState<number | ''>('')
	const [note, setNote] = useState('')
	const [reason, setReason] = useState<InventoryStockReason>('correction')
	...
	async function handleSubmit(event: FormEvent<HTMLFormElement>) {
		...
		if (quantity === '' || quantity < 0) {
			setErrors({ quantity: 'Quantity must be 0 or greater' })
			return
		}
		...
		try {
			...
			dispatch(showAlert({ message: 'Stock set successfully', severity: 'success' }))
		} catch (error) {
