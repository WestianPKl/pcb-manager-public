...
import type { CreateProjectInput } from '~/api/projectApi'
import { showAlert } from '~/stores/application-store'
import { useAppDispatch } from '~/stores/hooks'
import type { ApiError } from '~/api/api'
import type { ProjectClass } from '~/types/project/ProjectClass'
	...
	return (
		<form onSubmit={handleSubmit} className='flex flex-col gap-4'>
			...
			<button
				...
				{isSubmitting ? (isEdit ? 'Updating...' : 'Adding...') : isEdit ? 'Update Project' : 'Add Project'}
			</button>
		</form>
	)
}
